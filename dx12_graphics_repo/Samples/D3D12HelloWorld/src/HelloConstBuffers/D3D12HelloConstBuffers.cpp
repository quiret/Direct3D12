...
void D3D12HelloConstBuffers::LoadAssets()
{
	// Create a root signature consisting of a single CBV parameter.
	{
		...
		rootParameters[0].InitAsUnorderedAccessView(1);
	...
	// Create the pipeline state, which includes compiling and loading shaders.
	{
        ...
        ComPtr <ID3DBlob> errorMsgbuf;
        {
		    HRESULT res = D3DCompileFromFile(GetAssetFullPath(L"shaders.hlsl").c_str(), nullptr, nullptr, "VSMain", "vs_5_0", compileFlags, 0, &vertexShader, &errorMsgbuf);

            if ( res != S_OK )
            {
                const char *wbuf = (const char*)errorMsgbuf->GetBufferPointer();

                assert( res == S_OK );
            }
        }
        {
		    HRESULT res = D3DCompileFromFile(GetAssetFullPath(L"shaders.hlsl").c_str(), nullptr, nullptr, "PSMain", "ps_5_0", compileFlags, 0, &pixelShader, &errorMsgbuf);
    ...
    CD3DX12_CPU_DESCRIPTOR_HANDLE cbvDescriptor, uavDescriptor;
	{
		ThrowIfFailed(m_device->CreateCommittedResource(
			&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD),
			D3D12_HEAP_FLAG_NONE,
        ...
        m_commandList->ResourceBarrier( 1, &CD3DX12_RESOURCE_BARRIER::Transition(m_uavEndBuffer.Get(), D3D12_RESOURCE_STATE_GENERIC_READ, D3D12_RESOURCE_STATE_UNORDERED_ACCESS ) );
	}

    // Do some testing with the Intel dedicated memory.
    {
        ...
        heapDesc.SizeInBytes = sizeof( DWORD );

        ComPtr <ID3D12Heap> pHeap;

        ThrowIfFailed(
            m_device->CreateHeap( &heapDesc, IID_PPV_ARGS( &pHeap ) )
        );

        // Get the memory region of the heap.
        ID3D12Heap *heap = pHeap.Get();

        // For that we have to create a resource.
        ComPtr <ID3D12Resource> sampleBuffer;
        ...
        ThrowIfFailed(m_device->CreatePlacedResource( heap, 0, &resDesc, D3D12_RESOURCE_STATE_GENERIC_READ, NULL, IID_PPV_ARGS( &sampleBuffer ) ));

        // Get the memory pointer to the resource.
        ID3D12Resource *buf = sampleBuffer.Get();

        D3D12_GPU_VIRTUAL_ADDRESS gpuMemAddr = buf->GetGPUVirtualAddress();

        const char *gpuData = (const char*)gpuMemAddr;

        // Do a naive map attempt.
        char *mapData = NULL;

        ThrowIfFailed(buf->Map( 0, NULL, (void**)&mapData ));

        // Examine what we just did!
        __nop();
    }
	...
	// Create and record the bundle.
	{
        ...
        CD3DX12_GPU_DESCRIPTOR_HANDLE heapIter( m_cbvHeap->GetGPUDescriptorHandleForHeapStart() );
        
        m_bundle->SetGraphicsRootDescriptorTable(1, heapIter);

        heapIter.Offset( this->m_cbvUavDescriptorSize );

        D3D12_GPU_VIRTUAL_ADDRESS gpuMemAddr = m_uavEndBuffer.Get()->GetGPUVirtualAddress();

		m_bundle->SetGraphicsRootUnorderedAccessView(0, gpuMemAddr);
...
void D3D12HelloConstBuffers::OnUpdate()
{
    ...
    ConstantBuffer *gpuData = (ConstantBuffer*)m_pCbvDataBegin;
    DepthColorSortSample *gpuData2 = (DepthColorSortSample*)m_uavEndDataBegin;

    gpuData->screensize.x = this->m_width;
    gpuData->screensize.y = this->m_height;
